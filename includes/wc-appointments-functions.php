 ...
 */
function wc_appointment_duration_in_minutes( $start, $end, $pretty = true ) {
	if ( $start && $end ) {
...
function wc_appointments_get_total_available_appointments_for_range( $appointable_product, $start_date, $end_date, $staff_id = null, $qty = 1 ) {
	...
	if ( $appointable_product->has_staff() && ! $staff_id ) {
		...
		return $appointable_product->get_slots_availability( $start_date, $end_date, $qty, $staff_id, array(), $appointments );
	}
}
 ...
 * @return array
 */
function wc_appointments_get_posted_data( $posted = array(), $product = false, $get_data = false ) {
	if ( empty( $posted ) ) {
		$posted = $_POST;
	}
	...
	if ( $product->has_timezones() && $posted['wc_appointments_field_timezone'] && ! empty( $posted['wc_appointments_field_start_date_time'] ) ) {
	...
	if ( $get_data ) {
		return isset( $data[ $get_data ] ) ? $data[ $get_data ] : false;
	}
 ...
 * @since 4.5.0
 */
function wc_appointments_paginated_availability( $availability, $page = false, $records_per_page ) {
	$records = array();
	if ( false === $page ) {
