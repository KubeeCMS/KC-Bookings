...
class WC_Product_Appointment extends WC_Product_Appointment_Compatibility {
	...
	public function get_availability_rules( $for_staff = 0 ) {
		...
		if ( ! isset( $this->availability_rules[ $for_staff ] ) ) {
			...
			if ( $this->has_staff() && ! $for_staff ) {
				...
				$staff_rules  = $staff_object ? $staff_object->get_availability() : array();
				#print '<pre>'; print_r( $staff_rules ); print '</pre>';
			}
	...
	*/
	...
	public function get_slots_in_range_for_hour_or_minutes( $start_date, $end_date, $intervals, $staff_id, $scheduled, $get_past_times ) {
		...
		$default_appointable_minutes = $this->get_default_availability() ? range( $first_slot_time_minute, ( 1440 + $interval ) ) : array();
	...
	public function get_available_slots( $args ) {
		...
		#print '<pre>'; print_r( $original_available_qty ); print '</pre>';
 		...
 		if ( ! empty( $slots ) ) {
			...
			if ( ! empty( $appointments ) ) {
				foreach ( $appointments as $appointment ) {
					...
					if ( $product_staff
	 				...
	 				$product_scheduled[ $appointment_product_id ] = isset( $product_scheduled[ $appointment_product_id ] ) ? $product_scheduled[ $appointment_product_id ] : array();
			...
			$available_times = array_merge( $available_times, $this->get_slots_in_range( $start_date, $end_date, array( $interval, $base_interval ), $product_staff, isset( $product_scheduled[ $this->get_id() ] ) ? $product_scheduled[ $this->get_id() ] : $product_scheduled[0] ) );
	...
	*/
	...
	public function get_time_slots( $args ) {
		...
		if ( ! isset( $appointment_slots_transient_keys[ $this->get_id() ] ) ) {
			$appointment_slots_transient_keys[ $this->get_id() ] = array();
		}
						...
						if ( $quantity_available > 0 ) {
							if ( $quantity['scheduled'] ) {
								/* translators: 1: quantity available */
				 				$slot_html = "<li class=\"slot$selected\" data-slot=\"" . esc_attr( date( 'Hi', $display_slot ) ) . "\"><a href=\"#\" data-value=\"" . date_i18n( 'G:i', $display_slot ) . "\">" . date_i18n( wc_time_format(), $display_slot ) . " <small class=\"spaces-left\">" . $spaces_left . "</small></a></li>";
				 			} else {
				 				$slot_html = "<li class=\"slot$selected\" data-slot=\"" . esc_attr( date( 'Hi', $display_slot ) ) . "\"><a href=\"#\" data-value=\"" . date_i18n( 'G:i', $display_slot ) . "\">" . date_i18n( wc_time_format(), $display_slot ) . "</a></li>";
				 			}
