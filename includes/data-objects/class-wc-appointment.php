...
class WC_Appointment extends WC_Appointments_Data {
	...
	protected function status_transitioned_handler( $from, $to ) {
		...
		if ( $order ) {
			...
			$order->add_order_note( sprintf( __( 'Appointment #%1$d status changed from "%2$s" to "%3$s"', 'woocommerce-appointments' ), $this->get_id(), $from, $to ) );
		}
	...
	*/
	...
	public function passed_cancel_day() {
		...
		if ( false !== $appointment ) {
			$cancel_limit      = $appointment->get_cancel_limit();
			$cancel_limit_unit = $cancel_limit > 1 ? $appointment->get_cancel_limit_unit() . 's' : $appointment->get_cancel_limit_unit();
	...
	*/
	...
	public function get_start_date( $date_format = null, $time_format = null ) {
		if ( $this->get_start() ) {
			...
			if ( $this->is_all_day() ) {
				...
				return apply_filters( 'woocommerce_appointments_get_start_date_with_time', $get_start_date, $this );
			}
		}
		return false;
	}
	...
	public function get_end_date( $date_format = null, $time_format = null ) {
		if ( $this->get_end() ) {
			...
			if ( $this->is_all_day() ) {
				...
				return apply_filters( 'woocommerce_appointments_get_end_date_with_time', $get_end_date, $this );
			}
		}
		return false;
	}
	...
	public function get_duration( $pretty = true ) {
		return wc_appointment_duration_in_minutes( $this->get_start(), $this->get_end(), $pretty );
	}
	 ...
	 * @return string html formatted addon fields
	 */
	public function get_addons_from_order( $args = array() ) {
		$order = $this->get_order();
		if ( $order ) {
			$item_meta = '';
	 ...
	 * @return string html formatted addon fields
	 */
	public function get_addons_from_cart( $args = array() ) {
		if ( null === WC()->cart ) {
	...
	public function get_customer( $order = false ) {
		...
		// Works from WooCommerce 3.0 onward.
		$customer = class_exists( 'WC_Customer' ) && $this->get_customer_id() ? new WC_Customer( $this->get_customer_id() ) : 0;
		if ( $customer ) {
			$return['name']      = $customer->get_display_name();
			$return['full_name'] = trim( $customer->get_first_name() . ' ' . $customer->get_last_name() );
			$return['phone']     = $customer->get_billing_phone();
			$return['email']     = $customer->get_email();
			$return['user_id']   = $customer->get_id();
