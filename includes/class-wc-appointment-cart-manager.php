	...
	public function cart_item_removed( $cart_item_key ) {
		...
		if ( isset( $cart_item['appointment'] ) ) {
			...
			if ( $appointment && $appointment->has_status( 'in-cart' ) ) {
				...
				if ( isset( $this->log ) ) {
					$message = sprintf( 'Appointment ID: %s removed from cart by user ID: %s ', $appointment->get_id(), get_current_user_id() );
	...
	public function cart_item_restored( $cart_item_key ) {
		...
		if ( isset( $cart_item['appointment'] ) ) {
			...
			if ( $appointment->has_status( 'was-in-cart' ) ) {
				...
				if ( isset( $this->log ) ) {
					$message = sprintf( 'Appointment ID: %s was restored to cart by user ID: %s ', $appointment->get_id(), get_current_user_id() );
					$this->log->add( $this->id, $message );
				}
			}
		}
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['appointment'] ) ) {
				...
				if ( ! $appointment || ! $appointment->has_status( array( 'was-in-cart', 'in-cart', 'unpaid', 'paid' ) ) ) {
					...
					wc_add_notice( sprintf( __( 'An appointment for %s has been removed from your cart due to inactivity.', 'woocommerce-appointments' ), '<a href="' . get_permalink( $cart_item['product_id'] ) . '">' . get_the_title( $cart_item['product_id'] ) . '</a>' ), 'notice' );
				}
			}
		}
	}
