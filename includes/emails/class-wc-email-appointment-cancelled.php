...
class WC_Email_Appointment_Cancelled extends WC_Email {
	...
	public function get_default_additional_content() {
		return __( 'Please contact us if you have any questions or concerns.', 'woocommerce-appointments' );
	}
	...
	public function trigger( $appointment_id ) {
		...
		if ( $appointment_id ) {
			...
			if ( $this->object->get_order() ) {
				...
				$this->recipient = apply_filters( 'woocommerce_email_cancelled_recipients', $billing_email, $this->object );
			} else {
				...
				if ( $customer_id && $customer ) {
					$this->recipient = apply_filters( 'woocommerce_email_cancelled_recipients', $customer->user_email, $this->object );
				}
			}
