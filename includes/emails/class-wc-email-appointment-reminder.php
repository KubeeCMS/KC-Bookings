...
class WC_Email_Appointment_Reminder extends WC_Email {
	...
	/**
	 * Email reminder time options.
	 *
	 * @since  4.2.9
	 * @return array
	 */
	public function get_reminder_time_options() {
		// Hours.
		for ( $i = 1; $i <= 23; $i ++ ) {
			...
			$value .= '&nbsp;' . __( 'before the start date.', 'woocommerce-appointments' );
	...
	public function get_default_subject() {
		return __( '[{site_title}]: Reminder of your appointment for "{product_title}"', 'woocommerce-appointments' );
	}
	...
	public function get_default_heading() {
		return __( 'Appointment #{appointment_number} Reminder', 'woocommerce-appointments' );
	}
    ...
    public function init_form_fields() {
    	$this->form_fields = array(
			...
			'reminder_time'      => array(
				...
				'type'        => 'select',
				'description' => __( 'Sending time before the appointment is scheduled.', 'woocommerce-appointments' ),
				'default'     => '1 day',
				'class'       => 'email_time wc-enhanced-select',
				'options'     => $this->get_reminder_time_options(),
				'desc_tip'    => true,
			),
			'subject'            => array(
