 ...
 *
 * Last compatibility check: WPML Multilingual CMS 4.3.3
 * Last compatibility check: WooCommerce Multilingual 4.7.5
 */
class WC_Appointments_Integration_WCML {
	...
	public function __construct( SitePress $sitepress, woocommerce $woocommerce, woocommerce_wpml $woocommerce_wpml, wpdb $wpdb, WPML_Element_Translation_Package $tp ) {
		...
		if ( ! class_exists( 'Product_Addon_Display' ) && class_exists( 'WCML_Product_Addons' ) ) {
			...
			if ( is_callable( array( $this->product_addons, 'add_hooks' ) ) ) {
	...
	public function add_hooks() {
		add_action( 'woocommerce_appointments_after_appointment_pricing_base_cost', array( $this, 'wcml_price_field_after_appointment_pricing_base_cost' ), 10, 2 );
		add_action( 'woocommerce_appointments_after_appointment_pricing_cost', array( $this, 'wcml_price_field_after_appointment_pricing_cost' ), 10, 2 );
		add_action( 'woocommerce_appointments_after_staff_cost', array( $this, 'wcml_price_field_after_staff_cost' ), 10, 2 );
		add_action( 'woocommerce_appointments_after_staff_qty', array( $this, 'wcml_price_field_after_staff_qty' ), 10, 2 );
		add_action( 'woocommerce_appointments_after_appointments_pricing', array( $this, 'after_appointments_pricing' ) );

		add_action( 'save_post', array( $this, 'save_appointment_action_handler' ), 110 );
		add_action( 'wcml_before_sync_product', array( $this, 'sync_product_data' ), 10, 2 );

		add_filter( 'woocommerce_appointments_process_pricing_rules_cost', array( $this, 'wc_appointments_process_pricing_rules_cost' ), 10, 3 );
		add_filter( 'woocommerce_appointments_process_pricing_rules_base_cost', array( $this, 'wc_appointments_process_pricing_rules_base_cost' ), 10, 3 );
		add_filter( 'woocommerce_appointments_process_pricing_rules_override_slot', array( $this, 'wc_appointments_process_pricing_rules_override_slot_cost' ), 10, 3 );
		...
		add_action( 'wcml_update_extra_fields', array( $this, 'wcml_products_tab_sync_staff_and_availabilities' ), 10, 4 );

		$appointments_statuses = array(
			'unpaid',
		...
		foreach ( $appointments_statuses as $status ) {
			add_action( 'woocommerce_appointment_' . $status, array( $this, 'translate_transactional_appointment_email_texts' ) ); #translate emails
		}
		...
		add_action( 'woocommerce_appointment_pending-confirmation_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_email_texts' ), 9 );
		add_action( 'woocommerce_appointment_confirmed_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_email_texts' ), 9 );
		add_action( 'woocommerce_appointment_paid_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_email_texts' ), 9 );
		add_action( 'woocommerce_appointment_pending-confirmation_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_admin_email_texts' ), 9 );
		add_action( 'woocommerce_appointment_confirmed_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_admin_email_texts' ), 9 );
		add_action( 'woocommerce_appointment_paid_to_cancelled_notification', array( $this, 'translate_appointment_cancelled_admin_email_texts' ), 9 );

		add_action( 'woocommerce_admin_new_appointment_notification', array( $this, 'translate_new_appointment_email_texts' ), 9 );
		add_action( 'wc-appointment-confirmed', array( $this, 'translate_appointment_confirmed_email_texts' ), 9 );
		add_action( 'wc-appointment-reminder', array( $this, 'translate_appointment_reminder_email_texts' ), 9 );
		add_action( 'wc-appointment-follow-up', array( $this, 'translate_appointment_follow_up_email_texts' ), 9 );

		add_filter( 'wcml_email_language', array( $this, 'appointment_email_language' ) );

		add_filter( 'woocommerce_appointments_in_date_range_query_args', array( $this, 'appointments_date_range_query_args' ) );
		add_filter( 'woocommerce_appointments_for_objects_query_args', array( $this, 'appointments_date_range_query_args' ) );
		...
		add_filter( 'wc_apointments_check_appointment_product', array( $this, 'filter_check_appointment_product' ), 1, 3 );
			...
			foreach ( $currencies as $currency_code => $currency ) {
				...
				switch ( $field ) {
					...
					case 'wcml_wc_appointment_staff_qty':
						...
						echo '<div class="wcml_appointments_staff_slot" >';
						echo '<label>' . get_woocommerce_currency_symbol( $currency_code ) . '</label>';
						echo '<input type="number" step="1" name="wcml_wc_appointment_staff_qty[' . $staff_id . '][' . $currency_code . ']" class="wcml_appointments_custom_qty" value="' . $value . '" placeholder="0" />';
						echo '</div>';
						break;
	 ...
	 * @return bool
	 */
	private function update_appointment_staff_qty( $currencies = array(), $post_id = 0, $staff_qty = array() ) {
		if ( empty( $staff_qty ) ) {
			return false;
		}
		...
		$wc_appointment_staff_qtys = array();
	...
	public function filter_check_appointment_product( $return = true, $appointment_id, $product_id ) {
	...
	public function translate_emails_text_strings( $value, $object, $old_value, $key ) {
		...
		$emails_ids = array( 'admin_appointment_cancelled', 'admin_new_appointment', 'appointment_cancelled', 'appointment_confirmed', 'appointment_reminder' );
