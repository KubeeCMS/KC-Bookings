 ...
 *
 * Last compatibility check: WooCommerce Product Add-ons 3.0.5
 */
class WC_Appointments_Integration_Addons {
	...
	public function appointment_display( $position = 'after', $product = 0 ) {
		...
		if ( is_array( $product_addons ) && count( $product_addons ) > 0 ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				$on_top = isset( $addon['wc_appointment_show_on_top'] ) ? $addon['wc_appointment_show_on_top'] : false;
	...
	public function save_addon_options_in_admin( $order_id, $item_id, $product ) {
		...
		// $cart_item_data, $product_id, $variation_id, $quantity, $post_data = array();
		$addons = $GLOBALS['Product_Addon_Cart']->add_cart_item_data( array(), $product_id, $_POST, true );
	...
	public function adjust_appointment_cost( $appointment_cost, $product, $posted ) {
		// Get addon cost.
		$addon_cost = isset( $posted['wc_appointments_field_addons_cost'] ) ? $posted['wc_appointments_field_addons_cost'] : 0;
	...
	public function adjust_appointment_duration( $appointment_duration, $product, $posted ) {
		// Get addon duration.
		$addon_duration = isset( $posted['wc_appointments_field_addons_duration'] ) ? $posted['wc_appointments_field_addons_duration'] : 0;
